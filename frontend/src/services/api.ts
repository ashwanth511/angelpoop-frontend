...
const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || 'https://angelpoop-backend-eight.vercel.app';
...
export const api = {
  ...
  createToken: async (tokenData: TokenData) => {
    try {
      ...
      const response = await axios.post<ApiResponse<TokenData>>(`${API_BASE_URL}/api/create-token`, tokenDataWithoutId);
  ...
  createAgent: async (agentData: AgentData) => {
    try {
      ...
      const response = await axios.post<ApiResponse<AgentData>>(`${API_BASE_URL}/api/create-agent`, agentData);
  ...
  getAgent: async (tokenId: string) => {
    try {
      ...
      const response = await axios.get<ApiResponse<AgentData>>(`${API_BASE_URL}/api/agent/${tokenId}`);
  ...
  storeChat: async (tokenId: string, message: any) => {
    try {
      ...
      const response = await axios.post<ApiResponse<any>>(`${API_BASE_URL}/api/agent/${tokenId}/chat`, message);
  ...
  storeConversation: async (tokenId: string, conversation: any) => {
    try {
      ...
      const response = await axios.post<ApiResponse<any>>(`${API_BASE_URL}/api/agents/${tokenId}/conversations`, conversation);
  ...
  getTokens: async () => {
    try {
      ...
      const response = await axios.get<ApiResponse<TokenData>>(`${API_BASE_URL}/api/tokens`);
  ...
  getToken: async (id: string) => {
    try {
      ...
      const response = await axios.get<ApiResponse<TokenData>>(`${API_BASE_URL}/api/token/${id}`);
  ...
  updateTokenAddress: async (id: string, address: string) => {
    try {
      ...
      const response = await axios.patch<ApiResponse<TokenData>>(`${API_BASE_URL}/api/token/${id}/address`, { address });
  ...
  updateToken: async (data: { id: string } & Partial<TokenData>) => {
    try {
      ...
      if ('inPool' in data) {
        const response = await axios.patch<ApiResponse<TokenData>>(`${API_BASE_URL}/api/token/${data.id}/pool`, {
          inPool: data.inPool,
      ...
      const response = await axios.patch<ApiResponse<TokenData>>(`${API_BASE_URL}/api/token/${data.id}`, data);
